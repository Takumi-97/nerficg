...
namespace SPaGS::rasterization::oit_blend::kernels::inference {
        ...
        const bool output_chw)
    {
        ...
        float4 rgbd_premultiplied = make_float4(0.0f);
        ...
        if (inside) {
            // compute color
            float4 rgbd_pixel = make_float4(0.0f);
            if (alpha_sum >= config::min_alpha_threshold) {
                const float alpha = 1.0f - transmittance;
                rgbd_pixel = alpha * (1.0f / alpha_sum) * rgbd_premultiplied;
                rgbd_pixel.w = rgbd_pixel.w / alpha;
            } 
            // store results
            const int pixel_idx = width * pixel_coords.y + pixel_coords.x;
            if (output_chw) {
                const int n_pixels = width * height;
                image[pixel_idx] = __saturatef(rgbd_pixel.x);
                image[n_pixels + pixel_idx] = __saturatef(rgbd_pixel.y);
                image[2 * n_pixels + pixel_idx] = __saturatef(rgbd_pixel.z);
            }
            else {
                const int base_idx = 3 * pixel_idx;
                image[base_idx] = __saturatef(rgbd_pixel.x);
                image[base_idx + 1] = __saturatef(rgbd_pixel.y);
                image[base_idx + 2] = __saturatef(rgbd_pixel.z);
            }
            depths[pixel_idx] = rgbd_pixel.w;
        }
    }
