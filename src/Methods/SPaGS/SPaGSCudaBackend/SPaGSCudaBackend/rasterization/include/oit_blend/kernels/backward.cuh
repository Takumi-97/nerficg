...
namespace SPaGS::rasterization::oit_blend::kernels::backward {
        ...
        const bool use_distance_scaling)
    {
        ...
        // third row of dL_dMT is all zeros and thus not stored anywhere
        // fourth column of dL_dMT was already accounted for in blend backward
        const float3 dL_dMT_c1 = make_float3(grad_MT[primitive_idx], grad_MT[3 * n_primitives + primitive_idx], grad_MT[6 * n_primitives + primitive_idx]);
        const float3 dL_dMT_c2 = make_float3(grad_MT[n_primitives + primitive_idx], grad_MT[4 * n_primitives + primitive_idx], grad_MT[7 * n_primitives + primitive_idx]);
        const float3 dL_dMT_c3 = make_float3(grad_MT[2 * n_primitives + primitive_idx], grad_MT[5 * n_primitives + primitive_idx], grad_MT[8 * n_primitives + primitive_idx]);
        ...
        // scale gradients from MT1[:3], MT2[:3], and MT3[:3]
        ...
        // rotation/quaternion gradients from MT1[:3], MT2[:3], and MT3[:3]
        const Mat3x3 dL_dR = {
        ...
        const float near)
    {
        ...
        // setup shared memory
        __shared__ float3 collected_grad_color_partial[config::block_size_blend], collected_grad_alpha_c[config::block_size_blend];
        __shared__ float collected_grad_alpha_common[config::block_size_blend], collected_grad_alpha_a[config::block_size_blend];
        ...
        if (inside) {
            ...
            const float3 rgb = make_float3(
                pixel_grad_info[pixel_idx],
                pixel_grad_info[n_pixels + pixel_idx],
                pixel_grad_info[2 * n_pixels + pixel_idx]
            );
            const float alpha_sum_rcp = pixel_grad_info[3 * n_pixels + pixel_idx];
            const float transmittance = pixel_grad_info[4 * n_pixels + pixel_idx];
            const float alpha_final = 1.0f - transmittance;
            const float weight_final = alpha_sum_rcp * alpha_final;
            const float rgb_dot_grad_pixel = dot(rgb, grad_pixel);
            collected_grad_color_partial[thread_rank] = weight_final * grad_pixel;
            collected_grad_alpha_common[thread_rank] = -weight_final * alpha_sum_rcp * rgb_dot_grad_pixel;
            collected_grad_alpha_c[thread_rank] = weight_final * grad_pixel;
            collected_grad_alpha_a[thread_rank] = alpha_sum_rcp * transmittance * rgb_dot_grad_pixel;
        }
        ...
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            ...
            if (current_fetch_idx < tile_range.y) {
                ...
                current_MT1 = primitive_MT1[current_primitive_idx];
                current_MT2 = primitive_MT2[current_primitive_idx];
                current_MT3 = primitive_MT3[current_primitive_idx];
            ...
            for (uint pixel_idx_in_tile = 0u; pixel_idx_in_tile < config::block_size_blend; ++pixel_idx_in_tile) {
                ...
                if (current_inside) {
                    const float pixel_x = __uint2float_rn(current_pixel_coords.x) + 0.5f;
                    const float pixel_y = __uint2float_rn(current_pixel_coords.y) + 0.5f;
                    ...
                    const float4 plane_x_diag = current_MT1 * ray_plane_normal_x.x + current_MT3 * ray_plane_normal_x.z;
                    const float4 plane_y_diag = current_MT1 * ray_plane_normal_y.x + current_MT2 * ray_plane_normal_y.y + current_MT3 * ray_plane_normal_y.z;
                    ...
                    const float3 eval_point_view = make_float3(
                        dot(make_float3(current_MT1), eval_point_diag) + current_MT1.w,
                        dot(make_float3(current_MT2), eval_point_diag) + current_MT2.w,
                        dot(make_float3(current_MT3), eval_point_diag) + current_MT3.w
                    );
                    float depth = dot(eval_point_view, ray_direction);
                    if (depth <= near) continue;

                    // color gradient
                    dL_drgb_accum += collected_grad_color_partial[pixel_idx_in_tile] * alpha;

                    // opacity/position/MT gradients
                    const float dL_dalpha = collected_grad_alpha_common[pixel_idx_in_tile] + dot(current_rgb, collected_grad_alpha_c[pixel_idx_in_tile]) + collected_grad_alpha_a[pixel_idx_in_tile] / fmaxf(1.0f - alpha, config::one_minus_alpha_eps);
                    ...
                    dL_dMT1_accum += make_float3(dL_dMT1);
                    dL_dMT2_accum += make_float3(dL_dMT2);
                    dL_dMT3_accum += make_float3(dL_dMT3);
            ...
            if (current_fetch_idx < tile_range.y) {
                ...
                atomicAdd(&grad_MT[current_primitive_idx], dL_dMT1_accum.x);
                atomicAdd(&grad_MT[n_primitives + current_primitive_idx], dL_dMT1_accum.y);
                atomicAdd(&grad_MT[2 * n_primitives + current_primitive_idx], dL_dMT1_accum.z);
                atomicAdd(&grad_MT[3 * n_primitives + current_primitive_idx], dL_dMT2_accum.x);
                atomicAdd(&grad_MT[4 * n_primitives + current_primitive_idx], dL_dMT2_accum.y);
                atomicAdd(&grad_MT[5 * n_primitives + current_primitive_idx], dL_dMT2_accum.z);
