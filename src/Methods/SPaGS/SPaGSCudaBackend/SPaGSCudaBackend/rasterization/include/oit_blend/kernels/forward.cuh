...
namespace SPaGS::rasterization::oit_blend::kernels::forward {
        ...
        const float near)
    {
        ...
        float3 rgb_premultiplied = make_float3(0.0f);
        ...
        if (inside) {
            ...
            if (alpha_sum >= config::min_alpha_threshold) {
                const float alpha_sum_rcp = 1.0f / alpha_sum;
                rgb_pixel = (1.0f - transmittance) * alpha_sum_rcp * rgb_premultiplied;
                // store tail gradient info
                pixel_grad_info[pixel_idx] = rgb_premultiplied.x;
                pixel_grad_info[n_pixels + pixel_idx] = rgb_premultiplied.y;
                pixel_grad_info[2 * n_pixels + pixel_idx] = rgb_premultiplied.z;
                pixel_grad_info[3 * n_pixels + pixel_idx] = alpha_sum_rcp;
                pixel_grad_info[4 * n_pixels + pixel_idx] = transmittance;
            }
            // store results
