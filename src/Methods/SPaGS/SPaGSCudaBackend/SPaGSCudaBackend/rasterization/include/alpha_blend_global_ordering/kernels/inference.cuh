...
namespace SPaGS::rasterization::alpha_blend_global_ordering::kernels::inference {
        ...
        const bool use_median_depth)
    {
        ...
        __shared__ float collected_opacity[config::block_size_blend], collected_depth[config::block_size_blend];
        ...
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            ...
            for (int j = 0; !done && j < current_batch_size; ++j) {
                ...
                float pixel_depth = dot(eval_point_view, ray_direction);
                if (pixel_depth <= near) continue;
