...
namespace SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights {
    ...
    // ===== 修正点：関数名も変える =====
    template <int K>
    __global__ void __launch_bounds__(config::block_size_blend) update_contribution_cu(
    ...
    )
    {
        ...
        const float3 ray_direction = normalize(make_float3(
            sin_phi * cos_theta,
            sin_theta,
            -cos_phi * cos_theta
        ));
        ...
        __shared__ float4 collected_MT1[config::block_size_blend];
        __shared__ float4 collected_MT2[config::block_size_blend];
        __shared__ float4 collected_MT3[config::block_size_blend];
            ...
            current_fetch_idx += config::block_size_blend)
        {
            ...
            if (inside) {
                ...
                for (int j = 0; j < current_batch_size; ++j) {
                    ...
                    if (numerator_rho2 > config::max_cutoff_sq * denominator) continue;
        ...
        if (inside) {
            ...
            for (int core_idx = 0; core_idx < K; ++core_idx) {
                ...
                // 🔥 ここが論文コア
                const float weight = blending_weight * fabsf(sin_theta);

                atomicAdd(&contribution[primitive_idx], weight);
            }
        }
    }
