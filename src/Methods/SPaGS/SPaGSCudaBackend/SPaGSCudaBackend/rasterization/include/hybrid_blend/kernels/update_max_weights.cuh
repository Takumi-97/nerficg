...
namespace SPaGS::rasterization::hybrid_blend::kernels::update_max_weights {
        ...
        const float weight_threshold)
    {
        ...
        uint2 tile_range = tile_instance_ranges[group_index.y * grid_width + group_index.x];
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            ...
            if (inside) {
                ...
                for (int j = 0; j < current_batch_size; ++j) {
                    ...
                    alpha_sum_tail += alpha;
                    transmittance_tail *= 1.0f - alpha;
                }
            }
        }
        ...
        if (inside) {
            ...
            for (int core_idx = 0; core_idx < K; ++core_idx) {
                ...
                const float blending_weight = transmittance_core * alpha;
                transmittance_core *= 1.0f - alpha;
            ...
            // compute tail weight
            if (alpha_sum_tail >= weight_threshold) tail_weight = transmittance_core * (1.0f - transmittance_tail) * (1.0f / alpha_sum_tail);
        }
        // iterate over primitives again to write tail weights where necessary
        tile_range = tile_instance_ranges[group_index.y * grid_width + group_index.x];
        for (int n_points_remaining = tile_range.y - tile_range.x, current_fetch_idx = tile_range.x + thread_rank; n_points_remaining > 0; n_points_remaining -= config::block_size_blend, current_fetch_idx += config::block_size_blend) {
            block.sync();
            if (current_fetch_idx < tile_range.y) {
                const uint primitive_idx = instance_primitive_indices[current_fetch_idx];
            ...
            block.sync();
            if (inside && tail_weight >= weight_threshold) {
                const int current_batch_size = min(config::block_size_blend, n_points_remaining);
                for (int j = 0; j < current_batch_size; ++j) {
                    const float4 MT1 = collected_MT1[j];
                    const float4 MT2 = collected_MT2[j];
                    ...
                    // even if the fragment is in the core, the weight computed here is guaranteed to be smaller
                    const float blending_weight = alpha * tail_weight;
                    if (blending_weight < weight_threshold) continue;
