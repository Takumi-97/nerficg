...
#include "alpha_blend_first_k/kernels/update_max_weights.cuh"
...
void update_max_weights_k_templated(
    const dim3& grid,
    const dim3& block,
    const int K,
    Args&&... kernel_args)
{
    if (K >= 32) SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<32><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 16) SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<16><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 8) SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<8><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 4) SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<4><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 2) SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<2><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else SPaGS::rasterization::alpha_blend_first_k::kernels::update_max_weights::update_max_weights_cu<1><<<grid, block>>>(std::forward<Args>(kernel_args)...);
}

void SPaGS::rasterization::alpha_blend_first_k::update_max_weights(
    ...
    const float weight_threshold)
{
    ...
    std::visit([&](auto& per_instance_buffers) {
        ...
        update_max_weights_k_templated(grid, block, K,
            per_tile_buffers.instance_ranges,
            per_instance_buffers.primitive_indices.Current(),
            opacities,
            per_primitive_buffers.MT1,
