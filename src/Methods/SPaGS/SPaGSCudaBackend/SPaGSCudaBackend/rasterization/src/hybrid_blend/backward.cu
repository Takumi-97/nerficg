...
void blend_k_templated(
    ...
    Args&&... kernel_args)
{
    if (K >= 32) SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<32><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 16) SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<16><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 8) SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<8><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 4) SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<4><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else if (K >= 2) SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<2><<<grid, block>>>(std::forward<Args>(kernel_args)...);
    else SPaGS::rasterization::hybrid_blend::kernels::backward::blend_cu<1><<<grid, block>>>(std::forward<Args>(kernel_args)...);
}

void SPaGS::rasterization::hybrid_blend::backward(
